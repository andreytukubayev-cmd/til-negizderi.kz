        ...
        async function sendMessage() {
            ...
            if (waitingForName) {
                // Сохраняем имя и выходим
                saveUserNameFromMessage(message);
                input.value = '';
                return;
            }
            
            // Персонализируем сообщение, если есть имя пользователя
            let messageToSend = message;
            if (userName && !message.toLowerCase().includes(userName.toLowerCase())) {
                // Можно добавить контекст с именем, но не обязательно
                // messageToSend = `${userName} спрашивает: ${message}`;
            }
            ...
            try {
                const response = await fetch('/api/chat', {
                    ...
                    body: JSON.stringify({ 
                        ...
                        userName: userName // Отправляем имя на сервер (опционально)
                    })
                });
        ...
        function saveUserNameFromMessage(name) {
            if (name && name.trim().length > 0) {
                ...
                // Удаляем сообщение с вопросом о имени (оставляем только ответ пользователя)
        ...
        function formatMessage(text) {
            ...
            formatted = formatted.replace(/(Вариант для работы:|Как произнести:|Логика:)/g, '<strong>$1</strong>');
